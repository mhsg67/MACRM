...
case class InputJobDescription(JI: Long, UI: Int, DUR:Long, TS: List[InputTaskDescription], CS: List[InputConstraintDescription])
...
object JSONManager {
    ...
    def InputJobDescriptionToJobDescription(x: InputJobDescription) = {
        ...
        new JobDescription(x.JI, x.UI, x.DUR, x.TS.length, jobManagerContainerSpecification :: tasks)
    }
}