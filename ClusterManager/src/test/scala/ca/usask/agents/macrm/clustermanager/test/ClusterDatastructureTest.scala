...
class ClusterDatastructureTest extends UnitSpec {
    ...
    "getMatchCapablitiesOfNode" should "return list of node with related capablity" in {
        ...
        result.clusterNodesWithConstraint.length + result.clusterNodeWithoutConstraint.length should be (8)
        result.clusterNodeWithoutConstraint(0) should be(nId1)
        result.clusterNodeWithoutConstraint(1) should be(nId2)
        result.clusterNodesWithConstraint(0) should be((nId3, List(co2)))
        result.clusterNodesWithConstraint(1) should be((nId4, List(co2, co4)))
        result.clusterNodesWithConstraint(4) should be((nId7, List(co3)))
        result.clusterNodesWithConstraint(5) should be((nId8, List(co6, co2, co3)))

        cs.updateClusterStructure(-1, null, null, List((true, co7)))
        result = cs.getCurrentSamplingInformation(List(cp1, cp2, cp3))
        result.samplingRate should be(4)
        result.clusterNodesWithConstraint.length + result.clusterNodeWithoutConstraint.length should be(7)
        result.clusterNodeWithoutConstraint(0) should be(nId2)
        result.clusterNodesWithConstraint(0) should be((nId3, List(co2)))
        result.clusterNodesWithConstraint(1) should be((nId4, List(co2, co4)))
        result.clusterNodesWithConstraint(4) should be((nId7, List(co3)))
        result.clusterNodesWithConstraint(5) should be((nId8, List(co6, co2, co3)))
        ...
        result.clusterNodeWithoutConstraint should contain(nId1)
        result.clusterNodesWithConstraint should contain((nId3, List(co2)))
        result.clusterNodesWithConstraint should contain((nId4, List(co2, co4)))
        result.clusterNodesWithConstraint should contain((nId7, List(co3)))
        result.clusterNodesWithConstraint should contain((nId8, List(co6, co2, co3)))
        result.clusterNodeWithoutConstraint should not contain (nId2)

        cs.updateClusterStructure(-1, null, null, List((true, co7)))
        result = cs.getCurrentSamplingInformation(List(cp1, cp2, cp3))
        result.clusterNodesWithConstraint.length + result.clusterNodeWithoutConstraint.length should be(6)
        ...
        result.clusterNodesWithConstraint should contain((nId8, List(co6, co2, co3)))
        result.clusterNodeWithoutConstraint should not contain (nId2)
        result.clusterNodeWithoutConstraint should not contain (nId1)
    }
