...
class NodeManagerAgent extends Agent {
    ...
    val serverState = context.actorOf(Props[ServerStateAgent],name ="ServerStateAgent")
    val nodeMonitor = context.actorOf(Props(new NodeMonitorAgent(self,serverState)), name = "nodeMonitorAgnet")
    val containerManager = context.actorOf(Props(new ContainerManagerAgent(serverState)), name = "ContainerManagerAgent")
